<script>
    ...
    watch: {
        search: function () {
            ...
            if (!searchTerm) {
                 this.searchedChat = this.chats.filter(chat => chat.id_chat !== 0 && chat.id_utenti !== 0);
                this.searchedUser = this.chats.filter(chat => chat.id_chat === 0 || chat.id_utenti === 0);
                return;
            }
            // Filtra le chat e gli utenti in base al termine di ricerca
            const filtered = this.chats.filter(chat => {
                const name = (chat.nome || chat.username || '').toLowerCase();
                return name.includes(searchTerm);
            });

            // Popola searchedChat e searchedUser in base ai risultati
            this.searchedChat = filtered.filter(chat => chat.id_utenti !== 0);
            this.searchedUser = filtered.filter(chat => chat.id_chat !== 0);
        ...
        async forwardMessageToSelectedChats() {
            try {
                ...
                const selectedChatIds = this.selectedChat
                    ...
                    .map(chat => chat.id_chat);

                console.log(this.selectedUser)
                    
                const selectedUserIds = this.selectedUser
                    .filter(chat => chat.id_utenti !== 0)
                    .map(chat => chat.id_utenti);

                let response = await this.$axios.post(`/message/forward`,{
                    id_mes: this.id_mes, 
                    id_chat: selectedChatIds, 
...
<template>
    <div style="position: sticky; top: 0; z-index: 1000; padding: 10px; border-bottom: 1px solid #ccc;">
        <div class="d-flex">
            <form class="mt-3 flex-grow-1" @submit.prevent="getChats">
                ...
                <input type="text" class="form-control mb-2" id="username" placeholder="Insert Username" v-model="search" >
            </form>
            <button class="btn btn-secondary ms-1 mt-5 ml-auto h-100" @click="forwardMessageToSelectedChats">
                Submit
            </button>
        </div>
    </div>
    ...
    <div class="custom-scroll">
        <div v-for="chat in [...searchedChat, ...searchedUser]" :key="chat.id_chat" class="form-check">
